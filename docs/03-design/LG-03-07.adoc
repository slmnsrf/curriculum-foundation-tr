...
* arayüzlerin istenen özellikleri ve bunları tasarımda elde edebilirler (R1):
...
* arayüz ve uygulama arasındaki ayrım (R1):
** gerektiğinde uygulamalar değiştirilebilir.
...
** ikili (yalnızca makine tarafından okunabilir) veya metinsel (insan tarafından da okunabilir)
** durumsuz veya durumlu
** noktadan noktaya veya çok noktalı (yayın veya çoklu yayın)
** Fonksiyon çağrısı (örn. Remote Procedure Call) veya mesaj değişimi
** Toplu, istek/yanıt veya akış

* uzak {glossary_url}service[servis arayüzleri] için uygulama yaklaşımları, örneğin (R3):
** prosedür odaklı (örn. GraphQL veya WS-*/SOAP tabanlı web servisleri)
