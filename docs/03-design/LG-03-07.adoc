...
// tag::TR[]
[[LG-03-07]]
==== LG 03-07 [önceki LG 2-9]: Arayüzleri Tasarlamak ve Tanımlamak (R1-R3)

Yazılım mimarları, mimari yapı taşları arasında veya sistem ile harici öğeler arasındaki etkileşim için arayüzlerin kritik önemini bilir.
Bu tür arayüzleri tasarlayabilir ve belirleyebilirler.

Şunları bilirler:

* arayüzlerin istenen özellikleri bilirler ve bunları tasarımda elde edebilirler (R1):
** öğrenmesi kolay, kullanması kolay, genişletmesi kolay
** kötüye kullanması zor
** kullanıcılar veya bunları kullanan yapı taşları açısından işlevsel olarak eksiksiz.

* iç ve dış arayüzlere farklı davranma gerekliliği (R2)

* arayüz ve gerçekleştirme arasındaki ayrım (R1):
** gerektiğinde gerçekleştirmeler değiştirilebilir.

* arayüzlerin farklı özellikleri, örneğin (R3):
** Taşıma kanalları (örneğin: OSI 7 katmanlı modelinin parçası olarak TCP/IP, paylaşımlı bellek)
** dahili veya harici
** yerel veya uzaktan
** eşzamanlı veya eşzamansız
** ikilik (yalnızca makine tarafından okunabilir) veya metinsel (insan tarafından da okunabilir)
** durumsuz veya durumlu
** noktadan noktaya veya çok noktalı (yayın veya çoklu yayın)
** İşlev çağırma (örn. uzaktan yordam çağırma) veya mesaj değişimi
** Yığın, istek/yanıt veya akışlandırma

* uzaktan {glossary_url}service[hizmet arayüzleri] için uygulama yaklaşımları, örneğin (R3):
** yordam odaklı (örn. GraphQL veya WS-*/SOAP tabanlı web servisleri)
** kaynak odaklı (REST, REpresentational State Transfer)
