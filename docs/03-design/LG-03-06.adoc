...
Yazılım mimarları, yapı taşları arasındaki bağımlılıkları ve bağlılığı anlar ve bunları hedefli bir şekilde kullanabilir. Onlar:

* yapı taşlarının farklı bağımlılık türlerini bilir ve anlar (örneğin kullanım/delegasyon, mesajlaşma/olaylar, kompozisyon, oluşturma, kalıtım, zamansal bağlılık, veri, veri türleri veya donanım üzerinden bağlılık)
* bağımlılıkların bağlılığı nasıl artırdığını anlar
* en azından aşağıdaki bağlılık kategorilerini ayırt edebilir:
** statik ve dinamik bağlılık
** efferent ve afferent bağlılık
* dinamik bağımlılıklar lehine statik bağımlılıklardan vazgeçmenin altta yatan bağlılığı mutlaka azaltmadığını bilir
* bağlılığı belirleyebilir ve sonuçlarını değerlendirebilir
...
* bağlılığı azaltma veya ortadan kaldırma olanaklarını bilir ve uygulayabilir, örneğin:
** örüntüler
** temel tasarım ilkeleri
** bağımlılıkların dışsallaştırılması, yani somut bağımlılıkların kurulum veya çalışma zamanında tanımlanması, örneğin
{glossary_url}dependency-injection[Dependency Injection] kullanarak (R3) (ayrıca bkz. <<LG-03-08>>).
